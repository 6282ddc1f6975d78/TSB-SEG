...
def _prune_supervision_parameters(
    ...
    if removed:
        ...
        _log_info(
            f"Pruned supervision-controlled params from grid: {removed_list}"
        )
...
class GridPipeline:
    ...
    def _run_submitit(self, executor: Any) -> None:
        ...
        for index, combo in enumerate(self.parameter_grid):
            ...
            _log_info(f"Submitted: {job_name}")
    ...
    def _run_single_variant(self, variant_cfg: DictConfig, combo: Dict[str, Any], role: str = "worker") -> None:
        ...
        with mlflow.start_run(run_name=run_name) as run:
            _header(
                f"{variant_cfg.algorithm.name}  {_ARROW}  {variant_cfg.dataset.name}  [grid/{role}]",
                run_id=run.info.run_id,
            )
            if timeout_seconds is not None:
                _log_info(f"Time budget: {int(timeout_seconds)}s (~{timeout_seconds/3600:.2f}h)")
            ...
            try:
                ...
                _log_error(f"Grid run timed out after {timeout_display}s.")
